    ...
    public OpCommand() {
        ...
        addSyntax((sender, context) -> {
            ...
            if (sender instanceof Player player) {
                if (player.hasPermission(new Permission("phosphor.command.op"))) {
                    if (isOp(userName)) {
                        Component message = new KyoriTextFormatter("<dark_gray>┃ <red>OP <dark_gray>› <gray>The player " +
                                "<#419D78>" + userName + " <gray>is already an <red>Operator<dark_gray>.").getComponent();
                        player.sendMessage(message);
                        return;
                    }
                    PhosphorServerAPI.getInstance().opPlayer(userName);
                    Component message = new KyoriTextFormatter("<dark_gray>┃ <red>OP <dark_gray>› <gray>You have " +
                            "successfully added <#419D78>" + userName + " <gray>as <red>Operator<dark_gray>.").getComponent();
                    player.sendMessage(message);
                } else {
                    String noPermissionMessage = PhosphorServerAPI.getInstance().getServerProperties().getProperty("no-permission-message", String.class);
                ...
                if (isOp(userName)) {
                    sender.sendMessage("The player " + userName + " is already an Operator.");
                    return;
                }
                sender.sendMessage("You have successfully added " + userName + " as Operator.");
                PhosphorServerAPI.getInstance().opPlayer(userName);
            }
        }, argumentString);
    }
